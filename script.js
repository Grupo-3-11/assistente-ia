...
const selectors = {
  ...
  function handleCopy() {
    ...
    if (!text) {
      ui.showError("Nada para copiar, parça!");
      return;
    }
    ...
    navigator.clipboard
      ...
      .catch(() => {
        ui.showError("Não foi possível copiar, verifica as permissões!");
      });
  }
  ...
  async function getApiResponse(apiKey, question, model) {
        ...
        messages: [
          {
            role: "system",
            content:
              'Você é um assistente de IA que responde a perguntas usando gírias e expressões populares do Brasil, como "e aí", "beleza?", "demorô", "tá ligado?", "massa", "bacana", "mano", "parça", etc. Mantenha um tom informal e descontraído nas respostas.',
          },
          {
            role: "user",
            content: question,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error.message || `Erro na requisição: ${response.status}`
        );
      }
